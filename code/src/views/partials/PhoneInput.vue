  ...
  <div class="intl-phone-input allow-dropdown" v-on:mouseleave="hideList">
    <div class="flag-container" v-on:click.stop="toggeList">
      <div class="selected-flag">
        ...
        <div class="flag_text">(+{{ phone.code }})</div>
        <div v-bind:class="`iti-arrow${isVisiblePanel ? ' up' : ''}`"></div>
      </div>
      <ul class="country-list" v-if="isVisiblePanel">
        <li v-for="item in itemsData.frontItems" v-bind:class="`country${item.code === countryCode ? ' highlight' : ''}`"
          v-on:click.stop="setCode(item.code)" :key="item">
          <div class="flag-box">
        ...
        <li v-for="item in itemsData.otherItems" v-bind:class="`country${item.code === countryCode ? ' highlight' : ''}`"
          v-on:click.stop="setCode(item.code)"  :key="item">
          <div class="flag-box">
            <div v-bind:class="`iti-flag ${item.code}`"></div>
          </div>
    ...
    <!-- <input type="text" autocomplete="off" v-bind:name="name" v-on:input="handleChangePhoneNumber" v-bind:value="phone.number"
      @keyup="changeCountry" 
      v-bind:placeholder="intlData.example"
       /> -->
       <input class="block-phone-input" type="text" autocomplete="off" v-bind:name="name" v-on:input="handleChangePhoneNumber" v-bind:value="phone.number"
      @keyup="changeCountry" 
      placeholder="请输入电话号码"
       />
  </div>
...
<script>
  ...
  export default {
    ...
    data() {
      ...
      const needCode = has.call(this.availableData, code) ? code : Object.keys(this.availableData)[43]
...
<style lang="scss">
  ...
  .block-phone-input::-webkit-input-placeholder{
    color: #bbbdc3;
  }
  .block-phone-input:-moz-placeholder{
    color: #bbbdc3;
  }
  .block-phone-input::-moz-placeholder{
    color: #bbbdc3;
  }
  .block-phone-input:-ms-input-placeholder{
    color: #bbbdc3;
  } 
  .block-phone-input{
    visibility: hidden;
